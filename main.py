...
def set_layout(game_board_visability = True, app_setting_visability = False, game_perforance_visability = True):
    ...
    performance_layout = [
        [sg.Text("WPM:", font=("Any 20"), key="-WPM-"), sg.Text("Accuracy:", font=("Any 20"), key="-accuracy-"), sg.Text("Time:", key="-time_elapsed-", font=("Any 20")), sg.Text("Correct/Incorrect words: 0/0", key="-correct_incorrect_words-", font=("Any 20"))]
    ]

    gameboard_layout = [
        [sg.Text("Sentence to type:", font=("Any 20"))],
        [sg.Text(gametext, key="-displayed_gametext-", font=("Any 20")),],
    ...
    game_settings = [
        [sg.Text("Game settings", font=("Any 20"), size=(100, 1), justification="center")], 
        [sg.Text("Sentence length", font=("Any 20"), justification="center")],
        [sg.Radio("Random", 1, key="-random_sentence-", enable_events=True, default=True),
    ...
    app_settings = [
        [sg.Text("App settings", font=("Any 20"), size=(100, 1), justification="center")],
...
def get_sentence_length(event):
    ...
    get_sentence_dict = {
        ...
        "-long_sentence-": (15, 20),
        "-very_long_sentence-": (15, 20),
        "-extremely_long_sentence-": (15, 20)
    }
...
sentence_length_beginning = 0
sentence_length_end = 100
...
while True:
    ...
    if is_game_active == True:
        ...
        window["-correct_incorrect_words-"].update("Correct/Incorrect words: {0}/{1}".format(int(correct_words),int(incorrect_words)))
    ...
    if event == "Start New Game":
        ...
        gametextwords = gametext.split()
        gametextwordslenght = int(len(gametextwords))
        window["-displayed_gametext-"].update(gametext)
        userinputtext = ""
        pastuserinput = ""
        correct_words = 0
        incorrect_words = 0
